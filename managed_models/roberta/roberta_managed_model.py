...
class RobertaManagedModel(BaseManagedModel):
    def init_worker(self, model_class, model_cfg, model_state_dict):
        ...
        self.logger.debug("build model_class")
        model = model_class(cfg, task, model)
        ...
        use_half_model = True
        # # Enable full model loading only for boolq and qa
        # if (cfg.task._name == 'sentence_prediction' and 'boolq' in cfg.task.data) or \
        #         (cfg.task._name == 'question_answering' and 'phraseqa' not in cfg.task.data):
        #     use_half_model = False
    ...
    def init_master(self, model_class, model_cfg, model_state_dict):
    ...
    def construct_batches(self, data):
        import numpy as np
        min_batch_size = 64
        def convert_batch(cur_batch):
                ...
                if data['model'] == "span":
                    ...
                    batch = {
                        ...
                        "target" : np.array(targets).to(device),
                    }
                return batch
        samples = []
    ...
    def active_learning(self, data, head, epoch=1, lr=1e-6):
        ...
        for epoch_num in range(epoch):
            ...
            total_sentences = 0
            status = []
            batches = self.construct_batches(data)
            ...
            for batch_idx, batch in enumerate(batches):
                ...
                total_loss, sample_size, logging_output = self.criterion.forward(
                    self.model.model,
                    batch,
                )
            ...
            # preds = []   
            # for batch, sample_idx in zip(*self.construct_batches(data)):
            #     batch_pred = self.predict(
            #         head=head,
            #         tokens=collate_tokens(batch, pad_idx=1).numpy(),
            #         return_logits=True
            #         )  
            #     preds.extend(batch_pred) 
            # self.logger.info("preds:", preds)
    ...
    def save(self, path):
        ...
        return path
    @staticmethod
    def get_model_init_kwargs(model, head=None):
        kwargs = {
