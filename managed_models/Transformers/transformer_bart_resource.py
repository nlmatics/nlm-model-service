    ...
    def post(self):
        ...
        try:
            ...
            if len(prompts) > 0:
                ...
                model_max_token_length  = self.model.config.max_position_embeddings
        ...
        self.logger.info(
            f"Inference bart for {len(prompts)} prompts finished in {wall_time:.2f}ms, {wall_time/max(1,len(prompts)):.2f}ms per sample",
        )
