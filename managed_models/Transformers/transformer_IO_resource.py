...
class SpanResource(BaseTransformersResource):
    ...
    def get_outputs_from_logits(self, logits, tokens, *args, **kwargs):
        ...
        for token, logit in zip(tokens, logits):
            ...
            # get probabilities
            is_token_prob = F.softmax(logit.float(), dim=0)
            
           
            Idx.append(idx)
            probs.append(is_token_prob)
            
            # extract answer text
            answer = ""
            # has answer
            if torch.sum(idx):
                answer = self.model_manager.decode(token[idx.bool()])
