...
class _BaseStreamer:
    def __init__(self, *args, **kwargs):
        ...
        self.back_thread = threading.Thread(
            target=self._loop_collect_result,
            name="thread_collect_result",
        )
    ...
    def predict(self, batch):
        ...
        assert len(batch) == len(
            ret,
        ), "input batch size {} and output batch size {} must be equal.".format(
            len(batch),
            len(ret),
        )
        return ret
...
class _BaseStreamWorker:
    ...
    def _run_once(self):
        ...
        logger.info(
            "[gpu worker %d] run_once batch_size: %d start_at: %s spend: %s"
...
class Streamer(_BaseStreamer):
    def __init__(
        ...
        self.cuda_devices = cuda_devices or range(worker_num)
    ...
    def _setup_gpu_worker(self):
        for i in range(self.worker_num):
            ...
            if self.cuda_devices is not None:
                ...
                #     logger.error(f"Error parsing gpu_id = {gpu_id}. {e}")
                #     gpu_id = None
...
class ParallelModel(ManagedModel):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.logger = logging.getLogger(self.__class__.__name__)
        self.logger.setLevel(logging.INFO)

    def init_model(self, model_class, *args, **kwargs):
        self.logger.info(f"Init ParallelModel on {self.gpu_id}")

        self.parallel_model = True
        # init model
    ...
    def predict(self, job):
        # dummy batch_size of 1
        job = job[0]
        # convert to pytorch tensor
        job["tokens"] = collate_tokens(
            job["tokens"],
            pad_idx=1,
        )

        self.logger.info(f"{self.gpu_id} GPU recieved {len(job['tokens'])} tasks")
        gpu_time = default_timer()
        with torch.no_grad():
            outputs = self.model.predict(**job).cpu().detach().numpy().tolist()

        gpu_time = (default_timer() - gpu_time) * 1000
        self.logger.debug(
            f"GPU {self.gpu_id} runtime {gpu_time:.2f}ms for current batch",
        )
        # return a list to support service-streamer (dummy batch size of 1)
        return [outputs]


def predict_smart_batch(model, batches, **kwargs):
    res = []
    # use streamer for multi gpu
    if isinstance(model, Streamer):
        futures = []
        for batch in batches:
            # submit and get back the future
            job = kwargs.copy()
            job["tokens"] = batch
            future = model.submit([job])
            futures.append(future)

        # loop over future and get back the outputs
        for future in futures:
            # service-streamer return the result as a dummy batch size of 1. The actual results are the first element
            for output in future.result(timeout=WORKER_TIMEOUT)[0]:
                res.append(output)
    else:
        for batch in batches:
            job = kwargs.copy()
            job["tokens"] = collate_tokens(batch, pad_idx=1)
            # submit and get back the future
            outputs = model.predict(**job).cpu().detach().numpy().tolist()
            res.extend(outputs)
    return res
