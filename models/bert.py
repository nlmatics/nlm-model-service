...
class BertQA(Resource):
        ...
        for (example_index, example) in enumerate(examples):
            ...
            for (doc_span_index, doc_span) in enumerate(doc_spans):
                ...
                if example_index < 20:
                    ...
                    self.logger.debug("unique_id: %s" % (unique_id))
                    self.logger.debug("example_index: %s" % (example_index))
                    self.logger.debug("doc_span_index: %s" % (doc_span_index))
                    self.logger.debug("tokens: %s" % " ".join(tokens))
                    self.logger.debug(
                        "token_to_orig_map: %s"
                        % " ".join(
                    ...
                    if is_training and example.is_impossible:
                        self.logger.debug("impossible example")
                    if is_training and not example.is_impossible:
                        answer_text = " ".join(
                            tokens[start_position : (end_position + 1)],
                        )
                        self.logger.debug("start_position: %d" % (start_position))
                        self.logger.debug("end_position: %d" % (end_position))
                        self.logger.debug("answer: %s" % (answer_text))
    ...
    def get_final_text(
        self, pred_text, orig_text, do_lower_case, verbose_logging=False,
    ):
        ...
        tokenizer = self.basic_tokenizer  # BasicTokenizer(do_lower_case=do_lower_case)
        ...
        if start_position == -1:
            if verbose_logging:
                logging.warn(f"Unable to find text: '{pred_text}' in '{orig_text}'")
            return orig_text
        ...
        if orig_start_position is None:
            if verbose_logging:
                logging.warn("Couldn't map start position")
            return orig_text
        ...
        if orig_end_position is None:
            if verbose_logging:
                logging.warn("Couldn't map end position")
            return orig_text
    ...
    def create_examples(self, text_a, text_b):
        def is_whitespace(c):
        ...
        examples = []
        qas_id = 0
        for idx, a in enumerate(text_a):
            paragraph_text = text_a[idx]
            doc_tokens = []
            char_to_word_offset = []
    ...
    def get_predictions(
        ...
        self.logger.info("Writing predictions to: %s" % (output_prediction_file))
        self.logger.info("Writing nbest to: %s" % (output_nbest_file))
    ...
    def post(self):
        ...
        answers = self.run_qa(texts, questions)
    ...
    def run_qa(
        ...
        """Runs QuestionAnswering model on the given set of sentences & questions and returns the predictions.
        :param s_1: sentences
        :param s_2: questions
        :param max_seq_length: default 384
        :param doc_stride: default 128
        ...
        :return: predictions (answers to the questions)
        """
        eval_examples = self.create_examples(s_1, s_2)
        ...
        self.logger.info("  Num orig examples = %d" % (len(eval_examples)))
        self.logger.info("  Num split examples = %d" % (len(eval_features)))
        self.logger.info("  Batch size = %d" % (predict_batch_size))
        ...
        for input_ids, input_mask, segment_ids, example_indices in tqdm(
            eval_dataloader, desc="Evaluating", disable=False,
        ):
            if len(all_results) % 1000 == 0:
                self.logger.debug("Processing example: %d" % (len(all_results)))
